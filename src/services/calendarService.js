...
async function getThisWeekEvents(tokens) {
  ...
  try {
    const response = await calendar.events.list({ calendarId: 'primary', timeMin: monday.toISOString(), timeMax: sunday.toISOString(), singleEvents: true, orderBy: 'startTime' });
...
async function addEvent(taskData, tokens) {
  ...
  const event = { summary: `📌 ${taskData.title}`, description: taskData.notes || '', start, end, colorId };
  try {
...
async function markEventDone(eventId, tokens) {
  ...
  try {
    ...
    if (!event.summary.startsWith('✅')) event.summary = `✅ ${event.summary}`;
...
async function deleteEvent(eventId, tokens) {
  const auth = createOAuth2Client(tokens);
  const calendar = google.calendar({ version: 'v3', auth });
  try { await calendar.events.delete({ calendarId: 'primary', eventId }); }
  catch (error) { console.error('Calendar deleteEvent error:', error.message); throw error; }
}
